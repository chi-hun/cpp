...
class C_slave {
    ...
    public:
        ...
        C_slave(const C_slave& slave) : name(slave.name), position(slave.position), age(slave.age), grade(slave.grade) {};
        virtual ~C_slave(){};
...
class C_slave_h : public C_slave {
    ...
    public:
        ...
        C_slave_h(const C_slave_h& slave) 
...
class C_slave_list {
    ...
    public:
        C_slave_list(int alloc_slave) : alloc_slave(alloc_slave), current_slave(0) {
            slave_list = new C_slave*[alloc_slave];
        };
        ~C_slave_list();
        void add_slave(C_slave* slave);
        int slave_num() {return current_slave;};
        void print_info();
...
int main(void) {
    ...
    k_slave_list.add_slave(new C_slave_h("최모씨", "차장", 31, 4, 5));
